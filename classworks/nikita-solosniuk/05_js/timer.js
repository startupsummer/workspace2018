...
let start = null;
function startTimer () {
    if (start === null && timeToNum(timer.textContent) != 0)
...
function incTimer (num) {
    ...
    if (timeToNum(timer.textContent) < 10 && timeToNum(timer.textContent) % 2 != 0)
        timer.style.color = 'red';
    else timer.style.color = '#434a54';
    if (timeToNum(timer.textContent) == 0) stopTimer();
}
