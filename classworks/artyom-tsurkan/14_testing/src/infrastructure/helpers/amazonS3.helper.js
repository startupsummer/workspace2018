...
exports.copy = async (fileName, existingFileName, folder = constants.DEFAULT_S3_FILES_FOLDER) => {
  ...
  try {
    const newKey = folder ? `${folder}/${fileName}` : fileName;
    const existingKey = folder ? `${folder}/${existingFileName}` : existingFileName;
...
exports.downloadFile = async (fileName, folder = constants.DEFAULT_S3_FILES_FOLDER) => {
  const result = {
    value: {},
    error: null,
  };
...
exports.getTemporaryLink = (
  fileName,
  folder = constants.DEFAULT_S3_FILES_FOLDER,
  expiresSeconds = 30,
) => {
